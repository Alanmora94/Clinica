...
GetProximosTurnos(salas: Array<Isala>, turnos: Array<Iturno>): Array<Iturno>{
  ...
  //************* RECORRO CADA SALA BUSCANDO TURNOS

let auxahora : Date = new Date(ahora);
auxahora.setMinutes(auxahora.getMinutes() + 15)



if(auxahora.getHours() < 18 ){

        for (let index = 0; index < salas.length; index++) {
          const sala = salas[index];


            let losturnos = this.GetTurnoPorSala(turnos, sala.id);
            let tomado = false;
            let objTomado : Iturno;
            let FechaTomado: Date;


            for (let l = 0; l < losturnos.length; l++) {
              const turnos1 = losturnos[l];


              if(turnos1.estado == 'tomado'){

                tomado = true;
                objTomado = turnos1;
                let objFecha : any = turnos1.fecha;
                FechaTomado = new Date(Date.parse(objFecha))
                break;
              }
            ...
            if(tomado){
              ...
              let existe = false;
              let aux1 : Date = new Date (FechaTomado);



              while(!existe){

                  aux1.setMinutes(aux1.getMinutes() + 15);


                  for (let x = 0; x < losturnos.length; x++) {
                    const x1 : any = losturnos[x].fecha;
                    const x2 : Date = new Date( Date.parse(x1));

                    if( aux1.getMinutes() == x2.getMinutes() && aux1.getHours() == x2.getHours() ){

                      _lista.push(losturnos[x]);
                      existe =  true;
                      break;
                    }
              ...
              let entro = true;
              let ax2Ahora : Date = new Date(ahora);

              while(entro){

                for (let k = 0; k < losturnos.length; k++) {
                  const tur : any = losturnos[k].fecha;
                  const tem : Date = new Date(Date.parse(tur))

                  if(tem > ahora){

                      //let resta : Date = new Date(tem);

                      //resta.setMinutes(resta.getMinutes() - 1);

                      ax2Ahora.setMinutes(ax2Ahora.getMinutes() + 1);

                      if(ax2Ahora.getHours() == tem.getHours() && ax2Ahora.getMinutes() == tem.getMinutes()){

                        _lista.push(losturnos[k]);
                        entro =  false;
                        break;
